"""
bull_call_spread.py — Bull Call Spread (bullish, defined-risk).

Buys the ATM call and sells the next OTM call (ATM+1) on the same expiry. Net
debit; profit rises as the underlying moves up, capped at the short strike.
...
Usage:
    UPSTOX_ACCESS_TOKEN=xxx python bull_call_spread.py
    # paper trade:
    UPSTOX_SANDBOX_ACCESS_TOKEN=xxx SANDBOX=1 python bull_call_spread.py
"""
...
LEGS = [
    ("CE", 0, "BUY", 1),    # long  ATM call
    ("CE", 1, "SELL", 1),   # short ATM+1 call
]
...
def main():
    ...
    if input("Place this spread? (yes/no): ").strip().lower() != "yes":
        print("Cancelled.")
        return
